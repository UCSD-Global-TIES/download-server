...
const downloadPage = (url, path, startFn, endFn, errorFn) => {
    ...
    metaScrape(url)
        .then(function (metadata) {
            const filename = metadata.general.title;
            ...
            scrape({
                ...
                zipFolder(folderPath, `${folderPath}.zip`, function (err) {
                    if (err) {
                        errorFn(err);
                    } else {
...
const pageErrorFn = (err) => {
    console.log("Error!", err);
}
downloadPage("https://www.npmjs.com/package/find", "..", pageStartFn, pageEndFn, pageErrorFn)

// Can download any type of file (i.e. PDFs, images)
const downloadFile = (uri, path, startFn, endFn, errorFn) => {
...
// downloadFile("http://foersom.com/net/HowTo/data/OoPdfFormExample.pdf", ".", pageStartFn, pageEndFn, pageErrorFn)